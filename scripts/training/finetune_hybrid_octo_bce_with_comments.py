"""
Finetuning script for the Kinova goal-image Octo hybrid policy.

The checkpoint trained by this file keeps Octo's original diffusion action head
for the first six continuous end-effector action dimensions and adds a small BCE
head for the gripper dimension. This avoids treating the binary gripper command
as another continuous diffusion variable while preserving Octo's action-chunking
and goal-image conditioning pipeline.
"""
...
def load_tfds_dataset_statistics(
    ...
    """Load the most recent cached Octo/TFDS statistics JSON for this dataset."""
...
def make_lr_schedule(total_steps: int, warmup_steps: int, peak_lr: float):
    """Linear warmup followed by cosine decay, matching the finetuning recipe."""
...
def masked_mean(x, mask, eps=1e-8):
    """Mean over valid action entries only; invalid padded entries are ignored."""
...
def binary_stats(pred_bin, gt_bin, mask, eps=1e-8):
    """Precision/recall/F1 for the OPEN class under an action mask."""
...
class HybridTrainState(train_state.TrainState):
    """TrainState carrying the RNG used by dropout/diffusion during training."""
    rng: jax.Array
...
def main(_):
    ...
    def loss_fn(params, batch, rng, train=True):
        ...
        # Continuous arm loss via original Octo diffusion head.
        # cont_dim_mask zeros out the gripper dimension so diffusion only trains
        # xyz + rotational action components.
        ...
        # Label convention: gripper >= threshold is OPEN (1), else CLOSED (0).
    ...
    def save_hybrid_checkpoint(ckpt_dir, state, step, best_val_loss=None, overwrite=False, keep=3):
        """Save both Octo parameters and the separate BCE gripper-head params."""
        target = {
