"""
Offline per-episode evaluator for the Octo-arm + BCE-gripper checkpoint.

The script loads a hybrid checkpoint, evaluates it over a TFDS train/val split,
and writes per-episode metrics to CSV plus a summary JSON. Continuous arm
quality is measured with MSE/MAE, while gripper quality is measured with binary
OPEN/CLOSED metrics and transition statistics.
"""
...
class BinaryGripperHead(nn.Module):
    ...
    def __call__(self, emb):
        # emb: [B, D]
        # Output is one logit for each predicted action in the chunk.
...
def standardize_episode_from_rlds_np(ep):
    """Convert a raw RLDS episode to arrays with 7-D actions and done flags."""
...
def build_goal_only_task_from_episode(goal_rgb):
    """Package the episode goal image in the same schema used by Octo tasks."""
...
def build_obs_from_window(image_window, window_size):
    """Stack the current visual history into Octo observation format."""
    frames = [
...
def compute_binary_metrics(pred_prob, gt_raw, threshold=0.5, gt_open_threshold=0.5):
    """Compute OPEN/CLOSED classification and switching metrics."""
...
def main():
    ...
    # Build one batch through Octo's normal input pipeline. This gives
    # OctoModel.from_config the exact observation/task/action schema expected
    # by the finetuned checkpoint.
    dataset_kwargs = dict(
    ...
    def predict_step(octo_params, gripper_params, obs, task, rng):
        """Sample arm actions with Octo and append BCE gripper probabilities."""
