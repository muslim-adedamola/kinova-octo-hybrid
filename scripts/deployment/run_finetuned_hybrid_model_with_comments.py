"""
Live deployment script for the Kinova + Octo hybrid policy.

This script loads a finetuned Octo checkpoint whose continuous 6-DoF
end-effector actions are produced by Octo's diffusion action head, while the
gripper command is produced by a small BCE/sigmoid head. At runtime it:
  1. reads the latest RealSense RGB frame in a background thread,
  2. optionally applies a deployment crop matching the training crop,
  3. builds a goal-image-conditioned Octo observation/task batch,
  4. samples an action chunk,
  5. executes a short receding-horizon twist burst on the Kinova arm, and
  6. sends open/close commands to the gripper using hysteresis.

The default deployment mode is intended for real robot use, but the script
also supports shadow/debug operation when --execute_actions is not provided.
"""
...
class BinaryGripperHead(nn.Module):
    ...
    def __call__(self, emb):
        """Map the Octo readout embedding to one gripper logit per action step.

        Args:
            emb: Readout embedding with shape [B, D].

        Returns:
            Logits with shape [B, action_horizon]. Sigmoid(logits) is interpreted
            as probability of the gripper being OPEN, matching Octo convention.
        """
...
class RealSenseLatestRGB:
    ...
    def _run(self):
        """Continuously cache only the newest RealSense color frame.

        Keeping just the latest frame avoids a backlog when policy inference is
        slower than the camera frame rate. This is important for closed-loop
        control because stale visual observations can cause spatial bias.
        """
    ...
    def wait_for_new_frame(self, last_frame_counter=None, timeout_sec=1.0, poll_sec=0.005):
        """Block until a frame newer than last_frame_counter is available.

        In auto_receding_horizon mode this gives the robot one fresh visual
        observation after each executed burst before replanning.
        """
        start = time.time()
        while True:
...
def crop_bgr_image(image_bgr, crop_box):
    """Crop an OpenCV BGR image while clamping the ROI to image bounds."""
...
def startup_crop_review_loop(camera, initial_crop_box=None, crop_json_path=None):
    """Interactive startup tool for verifying the deployment crop.

    The model was trained on cropped frames, so deployment should use a crop
    that preserves the same object/table/framing distribution. Press R to
    reselect, F for full frame, and A/Enter to accept.
    """
...
def preprocess_bgr_to_rgb_256(image_bgr, crop_box=None):
    """Apply optional crop, convert BGR->RGB, and resize to Octo input size."""
    if crop_box is not None:
...
# -----------------------------------------------------------------------------
# Dataset statistics loading / model schema helpers
# -----------------------------------------------------------------------------
...
def extract_action_stats(model, dataset_stats=None):
    """Find Octo action normalization statistics from model or TFDS metadata.

    These statistics are passed to sample_actions so the continuous arm actions
    are returned in the same physical units used during data collection.
    """
    candidates = []
...
def extract_readout_tensor(x):
    """Normalize Octo transformer output into a [B, W, D] readout tensor.

    Octo versions may return a plain tensor, a token container, or a dictionary
    of readouts. The BCE gripper head needs a single tensor, so this helper keeps
    the deployment/eval scripts robust across minor Octo API differences.
    """
...
def build_goal_only_task(goal_image_path, goal_crop_box=None):
    """Load and package a single goal image as Octo task["image_primary"]."""
...
def send_twist_for_one_cycle(
    # Convert one model-predicted delta action into a Kinova Cartesian twist.
    # The model predicts deltas over a step; Kinova receives velocity commands,
    # so each delta is divided by dt and optionally clamped for safety.
    base,
    arm_action_6d,
    dt,
...
def get_burst_indices(start_idx, burst_len, horizon):
    start_idx = int(max(0, min(start_idx, horizon - 1)))
    burst_len = int(max(1, burst_len))
    end_idx = min(start_idx + burst_len, horizon)
...
def main():
    ...
    # Paths and model identity. checkpoint_path should point to the hybrid
    # checkpoint containing params["octo"] and params["gripper_head"].
    ...
    parser.add_argument(
        ...
        help="auto_receding_horizon: automatic burst->stop->settle->fresh-frame->replan. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Which arm action inside the predicted chunk to start executing from.",
    )
    parser.add_argument(
        "--gripper_chunk_index",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="How many consecutive arm chunk actions to execute before replanning.",
    )
    parser.add_argument(
        "--inter_pulse_gap_sec",
        type=float,
        default=0.0,
        help="Optional tiny pause between pulses inside one burst.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.08,
        help="How long to execute each twist pulse before moving to the next pulse or stopping.",
    )
    parser.add_argument(
        "--settle_time_sec",
        type=float,
    ...
    def predict_step(octo_params, gripper_params, obs, task, rng):
        """Run one closed-loop policy query.

        Returns a [B, H, 7] action chunk: first 6 dims from Octo diffusion,
        final gripper dim from the BCE/sigmoid head.
        """
    ...
    with DeviceConnection.createTcpConnection(robot_args) as router_tcp, DeviceConnection.createUdpConnection(robot_args) as router_udp:
        ...
        try:
            while True:
                ...
                # Fresh-frame gating
                # In automatic mode, do not immediately replan on the same
                # camera frame used before the previous arm burst. This reduces
                # latency-induced feedback errors during live deployment.
                ...
                arm_chunk_idx = max(0, min(args.arm_chunk_index, horizon - 1))
                gripper_chunk_idx = max(0, min(args.gripper_chunk_index, horizon - 1))

                arm_burst_indices = get_burst_indices(
                    start_idx=arm_chunk_idx,
                    burst_len=args.arm_burst_len,
                    horizon=horizon,
                )
                arm_burst_actions_6d = [action_chunk[i][:6].copy() for i in arm_burst_indices]
                ...
                print(
                    ...
                    f"arm_burst_indices={arm_burst_indices} gprob={gripper_prob:.3f}"
                )
                print("[ARM_BURST]", [np.round(a, 6).tolist() for a in arm_burst_actions_6d])
                ...
                for burst_i, arm_action_6d in enumerate(arm_burst_actions_6d):
                    ...
                    is_last_pulse = (burst_i == len(arm_burst_actions_6d) - 1)
                    ...
                    scaled_arm = send_twist_for_one_cycle(
                        ...
                        stop_after_pulse=is_last_pulse,
                    )

                    scaled_arm_last = scaled_arm

                    if (not is_last_pulse) and args.inter_pulse_gap_sec > 0:
                        time.sleep(args.inter_pulse_gap_sec)
                ...
                if args.show_window:
                    ...
                    txt1 = (
                        f"vx={scaled_arm_last[0]:+.4f} vy={scaled_arm_last[1]:+.4f} vz={scaled_arm_last[2]:+.4f} "
                        f"wx={scaled_arm_last[3]:+.2f} wy={scaled_arm_last[4]:+.2f} wz={scaled_arm_last[5]:+.2f}"
                    )
                    txt2 = (
                    ...
                    txt3 = (
                        ...
                        f"mode={args.control_mode} burst={len(arm_burst_indices)}"
                    )
